...
public class Sj_adeptor extends RecyclerView.Adapter<Sj_adeptor.ViewHolder>{
    ...
    public Sj_adeptor(Context context, List<Users> user, List<Task_Class> user1) {
        fcontext = context;
        fupload = user;
        fupload1 = user1;
            ...
            public void onClick(View view) {
                ...
                dr.child( fupload1.get( position ).getKey() ).removeValue().addOnCompleteListener( new OnCompleteListener<Void>() {
                    ...
                    public void onComplete(@NonNull Task<Void> task) {
                        Toast.makeText( fcontext, "Removed", Toast.LENGTH_SHORT ).show();
                        dr.keepSynced( true );
                        fcontext.startActivity( new Intent( fcontext, SJ_hall.class ) );
